...
PDF_ITEMS_FIELD = "notes" # must match your PDF form field
...
class BasePopup(tk.Toplevel):
    def __init__(self, master, categories, colors, title, on_done, columns_per_row=None, grouped_preview=False):
        ...
        # --- place category frames, wrapping at columns_per_row ---
        for idx, (cat, items) in enumerate(self.categories.items()):
            if self.columns_per_row and self.columns_per_row > 0:
                ...
                r, c = 0, idx  # original single-row behavior
    ...
    def get_selected_text(self) -> str:
        ...
        if self.grouped_preview:
            ...
            # --- Grouped, no numbering (for Items) ---
            ...
            for cat in self.categories.keys():
                if cat in grouped:
                    items_str = ", ".join(grouped[cat])
                    lines.append(f"{cat}: {items_str}")
            return "\n".join(lines)

        else:
            # --- Numbered list (for Repairs) ---
            lines = []
...
class RepairsPopup(BasePopup):
    def __init__(self, master, on_done):
        # Define categories and colors specific to Repairs
        categories = {
            "Replacement": [
...
class ItemsPopup(BasePopup):
    def __init__(self, master, on_done):
                # Define categories and colors specific to Items
        categories = {
            "LENS": [
                "Cut", "Delaminated", "Missing", "Damaged", "Hole", "Worn out", "Others",
            ],
            "CAP": [
                "Cut", "Missing", "Damaged", "Hole", "Dent marks", "Fell-off",
                "Leak at seam", "Worn out", "Others",
            ],
            "ARRAY (FirstCall)": [
                "Damaged", "Dead elements (used as it is)", "Weak elements", "Color noises",
                "No File", "Elements shortage to ground", "Others",
            ],
            "OIL LEAK": [
                "Seam", "Inner", "Bubbles", "Others",
            ],
            "ARRAY HOUSING": [
                "Damaged", "Scratches", "Broken", "Others",
            ],
            "SHAFT HOUSING": [
                "Halves splits", "Scratches", "Broken", "Damaged", "Stains", "Others",
            ],
            "CONNECTOR HOUSING": [
                "Paint scratches", "Scratched knob", "Knob fell-off", "Serial label damaged", "Others",
            ],
            "HOUSING STRAIN RELIEF": [
                "Cut", "Tear", "Cracks", "Worn out", "Pulled out shaft", "Discolored", "Others",
            ],
            "CONNECTOR STRAIN RELIEF": [
                "Cut", "Tear", "Cracks", "Worn out", "Detached from connector", "Discolored", "Others",
            ],
            "CABLE JACKET": [
                "Cut", "Tear", "Cracks", "Abrasion", "Damaged", "Scratches", "Twisted", "Bent",
                "Discolored","Pulled out strain", "Stiff", "Others",
            ],
            "CABLE (Functional Test)": [
                "Elements shortage to ground", "Liquid concentration", "Intermittent", "White noises", "Color noises",
                "Many dead elements", "No system", "Others",
            ],           
            "AIRSCAN": [
                "Dropouts", "Air bubbles", "Cloudy", "Uneven scanline", "Intermittent dropouts",
                "White noises", "Color noises", "No system", "3D/4D Error (Airscan cannot be captured)", "Others",
            ],
            "IMAGE": [
                "Shadows", "Poor resolution", "Intermittent dropouts", "Mirrored/Multiple images",
                "White noises", "No system", "3D/4D Error (Image cannot be captured)", "Others",
            ],
            "3D/4D": [
                "Couldn't find home", "Vibrating", "Cracking noise", "Array ball collapsed", 
        ...
        colors = {
            "LENS": "gainsboro",
            "CAP": "linen",
            "SHAFT HOUSING": "antique white",
            "ARRAY HOUSING": "papaya whip",
            "HOUSING STRAIN RELIEF": "blanched almond",
            "CABLE JACKET": "bisque",
            "CONNECTOR STRAIN RELIEF": "peach puff",
            "CONNECTOR HOUSING": "navajo white",
            "OIL LEAK": "mint cream",
            "ARRAY (FirstCall)": "azure",
            "AIRSCAN": "alice blue",
            "IMAGE": "light cyan",
            "3D/4D": "lavender blush",
            "CABLE (Functional Test)": "misty rose",
        }

        # Call the base class initializer
        super().__init__(master, categories, colors, "Select Items", on_done, columns_per_row=7, grouped_preview=True)
...
class App(tk.Tk):
    ...
    def prepare_pdf(self):
        ...
        original_file = "ev7.pdf"  # Default file
